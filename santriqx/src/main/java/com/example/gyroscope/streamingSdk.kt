...
class StreamingSDK(
    ...
    companion object {
         ...
         * Android 14+ — forces entire screen only (no "Record one app" option)
         */
        fun requestMediaProjectionPermission(activity: Activity, requestCode: Int) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                // Android 14+ — entire screen only, no single app option
                val config = android.media.projection.MediaProjectionConfig.createConfigForDefaultDisplay()
                activity.startActivityForResult(manager.createScreenCaptureIntent(config), requestCode)
                Log.d(TAG, "Android 14+ — entire screen only mode")
            } else {
                // Android 13 and below — single option anyway
         ...
         *
         * @return true if this was a MediaProjection result
         */
